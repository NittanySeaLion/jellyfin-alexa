...
[![Tests](https://github.com/NittanySeaLion/jellyfin-alexa/actions/workflows/test.yml/badge.svg)](https://github.com/NittanySeaLion/jellyfin-alexa/actions/workflows/test.yml)

A self-hosted custom Alexa skill for voice-controlled music playback from a [Jellyfin](https://jellyfin.org/) media server. This is original code, but it owes a real debt to the existing `infinityofspace/jellyfin-alexa-plugin` project — that project hadn't been updated in a while and got flaky once Alexa+ rolled out, so this one tries to move the ball forward. PRs with suggestions are welcome.

Also, the project looked at **[TODO: name the project]** and borrowed some API masking and usage of existing Alexa Music Player skill commands, but doesn't involve setting up a plugin in Jellyfin.

## How it works

Alexa itself is the audio player. To avoid Alexa's built-in Music domain hijacking the phrase, the skill uses a two-part invocation: first say "Alexa, open Jelly Fish Head." Alexa responds "Jellyfin skill ready. Say play or shuffle, followed by an artist, album, playlist, or song." Then say "play" or "shuffle" followed by a playlist, album, artist, etc.
...
3. Tells the Echo device to stream the audio via Alexa's [AudioPlayer interface](https://developer.amazon.com/en-US/docs/alexa/custom-skills/audioplayer-interface-reference.html), through this skill's own stream proxy (see [Technical details](#technical-details)) rather than a direct Jellyfin URL.
...
Supported voice commands: play by artist/album/playlist/song (either generically, via `PlayMusicIntent`, or with explicit type intents — `PlayArtistIntent`, `PlayAlbumIntent`, `PlayPlaylistIntent`), shuffle-play the same generic match via `ShuffleMusicIntent` ("shuffle \<query\>" — starts on a random track instead of always track 1, then continues in shuffled order), pause, resume, next, previous, stop, start over, shuffle the rest of an already-playing queue, and turn repeat on/off. Volume is handled natively by the Echo device. "What's playing" and multi-user account linking remain out of scope.
...
- The invocation name is **"jelly fish head"** (three words) — a two-word name ("jelly fin") worked but was occasionally misheard/misrouted; an earlier three-word name ("jelly fish hook") sounded too close to the common word "book" and had the same problem. Distinctive words that aren't close to ordinary English further reduce misrouting.
- The two-part flow — "open jelly fish head" first, then a bare "play"/"shuffle" follow-up — avoids Alexa's built-in Music domain, which otherwise claims any one-shot "play X on/by Y" phrasing before it ever reaches third-party skills (only Amazon's invite-only Music Skill API partners can intercept that pattern). Native "Alexa, open jelly fish head" (`LaunchRequest`) was unreliable in earlier testing; it's since proven reliable as the first half of this two-part flow.
...
src/
  ...
  handlers/
    ...
    playMusicHandler.js          PlayMusicIntent / ShuffleMusicIntent (generic, backend-disambiguated)
...
**Gotchas for contributors** (bugs already fixed once — don't reintroduce them):
...
4. **Restart the Echo device before testing** — power cycle it, or unplug/replug it — after enabling the skill for the first time and after every interaction-model rebuild. Echo devices can keep serving a stale skill/model until restarted, which looks identical to a broken invocation (misroutes to the built-in Music domain, or a flat "not supported on this device") with no indication the fix is just a restart.
5. **Real device, from the start**: enable the skill under the Alexa app's **More → Skills & Games → Your Skills → Dev** tab (same Amazon account as your Developer Console login), then test by talking to an actual Echo/Echo Dot. Use the two-part flow — "Alexa, open jelly fish head", then a bare "play \<something\>" or "shuffle \<something\>" follow-up — or the explicit one-shot "Alexa, ask jelly fish head to play \<something\>". Avoid "play X on jelly fish head" — Alexa's built-in Music domain claims "play X on Y" phrasing before it ever reaches third-party skills (this isn't a bug in the interaction model, it's a platform-level reservation; only Amazon's invite-only Music Skill API partners can intercept that pattern).
6. **Unit tests**: `npm test` runs a couple of lightweight tests (Node's built-in test runner) covering the stream URL builder and the queue store logic.
...
This repo is public. Avoid committing anything specific to your environment — real Jellyfin URL/IP, API key, user id, or public hostname; only placeholders appear in `.env.example` and `skill-package/skill.json`. Keep your real `.env` local (it's git-ignored) and double-check `git diff --staged` before pushing any config changes.
